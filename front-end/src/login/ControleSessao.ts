...
export default new class ControleSessao{
    ...
    getChamadoInfo(){
        const sessionTicket = sessionStorage.getItem('dados_chamado')

        if (sessionTicket == null){
            return null
        }

        const userInfo: string = JSON.stringify(sessionTicket)
        return sessionTicket
    }

    getChamadoId(){
        const chamadoId = this.getChamadoInfo()?.split(',')[0].split(':')[1].replaceAll('"', '')
        return chamadoId
    }
}
