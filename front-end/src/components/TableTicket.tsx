...
export function TableTicket() {
  ...
  const [chamadoId, setChamadoIds] = useState('')
        ...
        <Container>
          ...
          <InputGroup className="m-auto input-content" size="lg">
            <FormControl
              ...
              type="text"
              value={busca}
              onChange={(ev: any) => setBusca(ev.target.value)}
            />
          ...
          <Table responsive="xl" size="xl" hover bordered className="tableChamados">
            ...
            <tbody>
              {chamadosFiltrados.map((chamado: any) => (
                <tr key={chamado["id"]} onClick={() => navigate('/page/ticketlife')}

                 onClickCapture={() => {setChamadoIds(chamado["id"]); setPrioridade(chamado["prioridade"]); setTitulo(chamado["titulo"]);
                  setEquipamento(chamado["equipamento"]); setStatus(chamado["status"]); ControleSessao.setChamado(chamado)}}>
